...
export async function scanSyntheticSymbol({symbol, name, family, granularity = 60, client = new DerivClient()} = {}) {
  if (![60,300,900,3600].includes(granularity)) throw new Error('Intervalo inválido.');
  if (typeof symbol !== 'string' || !symbol) throw new Error('Símbolo inválido.');
  try {
    ...
    const data = await client.request({ticks_history:symbol, count:SCAN_COUNT + 1, end:boundary - 1, style:'candles', granularity});
    const candidate = scoreSymbol({symbol, display_name:name || symbol}, normalizeCandles(data.candles, boundary, granularity));
    return {...candidate, family, boundary, granularity};
